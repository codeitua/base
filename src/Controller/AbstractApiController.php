...
abstract class AbstractApiController extends AbstractRestfulController
{
    ...
    protected $acceptCriteria = [
        ...
        // 'application/xml',
        // ],
        'Laminas\\View\\Model\\JsonModel' => ['application/json', 'text/html'],
    ];
    public function ready()
    {
        ...
        if ($origin) {
            // allow API usage from any hosts
            $this->getResponse()->getHeaders()->addHeaderLine('Access-Control-Allow-Origin', $origin->getFieldValue());
        }
        $requestHeaders = $this->request->getHeader('Access-Control-Request-Headers');
        if ($requestHeaders) {
            // allow any headers asked
            $this->getResponse()->getHeaders()->addHeaderLine('Access-Control-Allow-Headers', $requestHeaders->getFieldValue());
        }
        ...
        if (strtoupper($this->request->getMethod()) == 'OPTIONS') {
            return true;
            // we should return http 200 with empty body
        }
        try {
    ...
    */
    public function returnData($data = null, $httpStatusCode = 200)
    {
        ...
        $model->setOption('prettyPrint', defined('DEBUG') && DEBUG);
        return $model;
    }
    /**
